...
export default function AuthPage() {
  ...
  useEffect(() => {
    if (mode !== "patient" || pin.length !== 4) return;
    const timeout = setTimeout(submit, 100);
  ...
  return (
    <main className="flex min-h-dvh flex-col items-center justify-center bg-gradient-to-b from-rose-50 to-white px-6">
      <div className="w-full max-w-xs">
        ...
        <button
          onClick={() => {
            setMode(mode === "patient" ? "admin" : "patient");
            setPin("");
            setError("");
          }}
          className="w-full text-center text-sm text-gray-400 hover:text-gray-600 py-2"
        >
          {mode === "patient" ? "Admin login →" : "← Back to Damilola's login"}
        </button>
      </div>
    </main>
  );
